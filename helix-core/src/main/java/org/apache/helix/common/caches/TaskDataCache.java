...
public class TaskDataCache extends AbstractDataCache {
  ...
  private Map<String, String> _initialContextStrings = new HashMap<>();
  ...
  private final static ZkSerializer SERIALIZER = new ZNRecordJacksonSerializer();
  ...
  private void refreshContexts(HelixDataAccessor accessor) {
    ...
    for (int i = 0; i < contexts.size(); i++) {
      ...
      if (context != null && context.getSimpleField(NAME) != null) {
        ...
        _initialContextStrings.put(context.getSimpleField(NAME),
            new String(SERIALIZER.serialize(context)));
      } else {
        _contextMap.put(childNames.get(i), context);
        _initialContextStrings.put(childNames.get(i),
            new String(SERIALIZER.serialize(context)));
  ...
  public void updateJobContext(String resourceName, JobContext jobContext) {
    if (_initialContextStrings.get(resourceName) == null
        || !SERIALIZER.deserialize(_initialContextStrings.get(resourceName).getBytes())
            .equals(jobContext.getRecord())) {
  ...
  public void updateWorkflowContext(String resourceName, WorkflowContext workflowContext) {
    if (_initialContextStrings.get(resourceName) == null
        || !SERIALIZER.deserialize(_initialContextStrings.get(resourceName).getBytes())
            .equals(workflowContext.getRecord())) {
